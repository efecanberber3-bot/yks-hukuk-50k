...
function assistantInsight(){
 ...
 return {h:'Kontrol sende.',p:read===null?`Bugün ${score}/100. Henüz yeterli performans verisi yok; bugün veri üretmeye odaklan. Hedefe gidişte ana iş: planı uygula, ilk denemeni gir ve konu ilerlemeni kaydet.`:`Bugün ${score}/100. Readiness ${read}/100. Hedefe gidişte ana iş: planı tamamla, veri gir, yarın için tek kritik eksiği işaretle.`,a:[['today','Bugünün planı'],['discipline','Günü değerlendir']]};
}
function renderDashboard(){
 ...
 const ai=assistantInsight();$('#assistantHeadline').textContent=ai.h;$('#assistantText').textContent=ai.p;$('#assistantActions').innerHTML=ai.a.map(([v,t])=>`<button data-route="${v}">${t}</button>`).join('')+`<div class="assistant-readiness ${rd.value===null?'is-pending':''}"><span>${rd.value===null?'VERİ DURUMU':'READINESS'}</span><b>${rd.label}</b><small>${rd.value===null?rd.sub:`${daysToExam()} gün • hedef ≤${state.settings.targetRank.toLocaleString('tr-TR')}`}</small></div>`;
...
function renderMock(){
 ...
 $('#mockKpis').innerHTML=`${[['Toplam',m.length,'deneme'],['TYT ort.',avg(t).toFixed(2),'net'],['AYT EA ort.',avg(a).toFixed(2),'net'],[readiness()===null?'Veri durumu':'Readiness',readiness()===null?'—':readiness(),readiness()===null?'bekleniyor':' / 100']].map(x=>`<div class="kpi"><span>${x[0]}</span><strong>${x[1]}</strong><small>${x[2]}</small></div>`).join('')}`;
...
const focus={total:1500,left:1500,running:false,handle:null,taskId:null,previousView:'dashboard'};
...
bind('focusTaskList','click',e=>{const btn=e.target.closest('[data-focus-task]');if(!btn)return;const d=ensureDay(),t=d.tasks.find(x=>x.id===btn.dataset.focusTask);if(!t)return;focus.taskId=t.id;$('#focusTargetLabel').textContent=t.title;$('#focusSelected').textContent=t.title;const suggested=Math.min(90,Math.max(25,Math.round((t.minutes||25)/5)*5));const dur=$('#focusDuration');if(dur){const allowed=[25,50,75,90];dur.value=String(allowed.includes(suggested)?suggested:(suggested<38?25:suggested<63?50:suggested<83?75:90))}setFocusTimer(Number($('#focusDuration')?.value||suggested));$('#focusTaskDialog')?.close();renderFocusTaskPicker();updateFocusStatus()});
