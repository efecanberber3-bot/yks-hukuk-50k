...
function goalSetupRender(){
 const t=$('#goalTrack'),p=$('#goalProgram'),custom=$('#goalCustomProgram');if(!t||!p)return;
 t.value=state.settings.scoreType||'EA';const opts=goalCatalog[t.value]||goalCatalog.EA;p.innerHTML=opts.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join('');p.value=opts.includes(state.settings.targetProgram)?state.settings.targetProgram:'Diğer / Kendim yazacağım';
 ...
 const rank=$('#goalRank');if(rank)rank.value=String(state.settings.targetRank||30000);
 const chips=$('#goalRankPresets');if(chips)chips.innerHTML=rankPresets.map(n=>`<button type="button" class="rank-chip ${Number(rank?.value)===n?'active':''}" data-rank-preset="${n}">${n>=1000?(n/1000)+'K':n}</button>`).join('');
}
...
function renderMockIntelligence(){
 ...
 const topicEl=$('#mockIntelTopic'),topicText=$('#mockIntelTopicText');if(top){topicEl.textContent=top[0];topicText.textContent=`${top[1]} hata kaydı. Bu alanı sonraki planın ilk sıralarına taşı.`}else{topicEl.textContent='Veri bekleniyor';topicText.textContent='Bir denemeye ait ilk hatanı konu ve hata tipiyle kaydet.'}
 const trendFor=a=>a.length>=2?Number((a.at(-1).net-a[0].net).toFixed(2)):null;const tt=trendFor(latestMocks('TYT').slice(-3)),aa=trendFor(latestMocks('AYT EA').slice(-3));let trend='—',trendText='Henüz yeterli deneme yok.';if(tt!==null||aa!==null){const bits=[];if(tt!==null)bits.push(`TYT ${tt>0?'+':''}${tt}`);if(aa!==null)bits.push(`AYT ${aa>0?'+':''}${aa}`);trend=bits.join(' • ')+' net';trendText='Son üç denemedeki yaklaşık değişim.'}$('#mockIntelTrend').textContent=trend;$('#mockIntelTrendText').textContent=trendText;
 let action='Veri topla',actionText='İlk denemeni ve yanlışlarını kaydet; koç motoru sinyali güçlensin.';if(top){action=`${top[0]} tekrar`;actionText=`${top[1]} hata kaydı nedeniyle konuya özel soru + yanlış analizi öneriliyor.`}if(aa!==null&&aa<0){action='AYT Matematik öncelik';actionText='AYT trendi geriliyorsa matematik çalışma ve hata analizi hacmini artır.'}$('#mockIntelAction').textContent=action;$('#mockIntelActionText').textContent=actionText;
}
...
setFocusTimer(10);ensureDay();renderAll();setTimeout(maybeOpenOnboarding,250);
