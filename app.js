...
function applyTheme(){document.documentElement.dataset.theme=state.theme;document.documentElement.dataset.accent=state.accentTheme||'lime';$('#themeBtn').textContent=state.theme==='dark'?'☼':'☾'; $$('.accent-option').forEach(x=>x.classList.toggle('selected',x.dataset.accent===state.accentTheme));const cur=accentThemes[state.accentTheme]||accentThemes.lime;document.documentElement.style.setProperty('--accent',cur[0]);document.documentElement.style.setProperty('--accent2',cur[1]);}
...
bind('themeBtn','click',()=>{state.theme=state.theme==='dark'?'light':'dark';save();applyTheme()});bind('accentThemeBtn','click',()=>{$('#accentThemeMenu')?.classList.toggle('open')});document.addEventListener('click',e=>{const menu=$('#accentThemeMenu');if(!menu)return;if(menu.contains(e.target)||e.target.id==='accentThemeBtn')return;menu.classList.remove('open')});document.addEventListener('click',e=>{const opt=e.target.closest('[data-accent]');if(!opt)return;state.accentTheme=opt.dataset.accent;save();applyTheme();$('#accentThemeMenu')?.classList.remove('open');toast(`${opt.dataset.label||'Tema'} seçildi.`)});
const sidebar=$('#sidebar'), mobileMenu=$('#mobileMenu'), sidebarBackdrop=$('#sidebarBackdrop');
const syncSidebarUi=()=>{const isOpen=sidebar?.classList.contains('open'); if(mobileMenu){mobileMenu.textContent=isOpen?'←':'☰'; mobileMenu.setAttribute('aria-label',isOpen?'Menüyü kapat':'Menüyü aç'); mobileMenu.setAttribute('title',isOpen?'Menüyü kapat':'Menüyü aç'); mobileMenu.classList.toggle('is-open',!!isOpen);} sidebarBackdrop?.classList.toggle('show',!!isOpen); if(sidebarBackdrop) sidebarBackdrop.setAttribute('aria-hidden',String(!isOpen));};
const closeSidebar=()=>{sidebar?.classList.remove('open'); syncSidebarUi()};
bind('mobileMenu','click',()=>{sidebar?.classList.toggle('open'); syncSidebarUi()});
